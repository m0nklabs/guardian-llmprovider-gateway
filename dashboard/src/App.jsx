...
const StatCard = ({ title, value, unit, icon: Icon, trend }) => (
  <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl shadow-lg">
    ...
    {trend && (
      <div
        className={`text-xs mt-2 ${trend > 0 ? "text-emerald-500" : "text-rose-500"}`}
      >
        {trend > 0 ? "+" : ""}
        {trend}% from average
      </div>
...
const ViewToggle = ({ view, setView }) => (
  <div className="flex gap-2 rounded-xl border border-slate-800 bg-slate-900 p-1">
    ...
    >
      <Shield className="w-4 h-4" /> Capture
    </button>
  </div>
);
...
const App = () => {
  ...
  const [apiKey, setApiKey] = useState(
    () => localStorage.getItem("guardian_key") || "",
  );

  // When switching to capture view, persist key
  const saveApiKey = (val) => {
    setApiKey(val);
    localStorage.setItem("guardian_key", val);
  };
  ...
  const processedData = useMemo(() => {
    ...
    const contextSizes = [...new Set(filtered.map((d) => d.context_size))].sort(
      (a, b) => a - b,
    );
  ...
  const visibleModels = processedData.models.filter((m) =>
      ...
      <div className="min-h-screen bg-slate-950 p-8 font-sans">
        <header className="mb-8 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <input
              type="password"
              placeholder="Guardian API Key"
              value={apiKey}
              onChange={(e) => saveApiKey(e.target.value)}
              className="px-4 py-2 rounded-lg bg-slate-900 border border-slate-800 text-slate-300 text-sm w-48 focus:outline-none focus:border-indigo-500"
            />
            <ViewToggle view={view} setView={setView} />
          </div>
        </header>
        <CapturePanel apiKey={apiKey} />
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-950 p-8 font-sans">
      <header className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 mt-2">
            Performance analysis across context windows
          </p>
        </div>
        <div className="flex items-center gap-4">
          <ViewToggle view={view} setView={setView} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <StatCard
          title="Avg Load Time"
          value={(
            rawData.reduce((acc, curr) => acc + (curr.load_time_seconds || 0), 0) /
            (rawData.length || 1)
          ).toFixed(2)}
          unit="sec"
          icon={Clock}
        ...
        <div className="lg:col-span-3 space-y-8">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
            <h3 className="text-xl font-semibold text-white mb-6">
              Inference Speed vs Context Size
            </h3>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
            <h3 className="text-xl font-semibold text-white mb-6">
              Generated Response Length (Characters)
            </h3>
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={processedData.chartData}>
