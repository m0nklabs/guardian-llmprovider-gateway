...
- **Guardian 2.0 masterplan (PLAN 2026-08-26, goedgekeurd)** → `@docs/IMPLEMENTATION_PLAN.md` (canoniek) + GitHub issue #1. Fases: F0 foundation + F1 file register → **gebouwd (PR #2)**; F2 → **gemerged (PR #7)**; F3 → **gemerged (PR #8)**; F4 → **gemerged (PR #10)**; F5 caretaker + local passief → **in uitvoering** (caretaker fases A–D + PR #6 gemerged in `m0nklabs/caretaker-llamacpp`, gateway-wiring tranche 1 PR #11 gemerged, tranche 2 `caretaker_runtime.py` merge-klaar — PR #12, wacht op human merge); F6 Windows/homelab + F7 cut-over → **open**.
...
- Org-level pool van 4 self-hosted runners (`m0nklabs-runner-1..4`) op host `ai-node`, alle **GPU-capabel** (label `gpu`); **nooit een aparte runner per project**.
