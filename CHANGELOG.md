...
### Fixed
- Restored the proven multimodal runtime for `Qwen3.6-35B-A3B-Heretic-Native-MTP-Preserved` in `models.yaml` after drift had pushed the vision path back to `vision_ngl: 99` with `vision_tensor_split: "0.50,0.50"`, which caused Guardian 503 auto-reload failures and llama-server segfaults on image requests. The vision runtime is back on the validated `262144 / 36 / 0.55,0.45` shape.
