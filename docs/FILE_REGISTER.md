...
| `config/providers/ai-node-local.settings.yaml` | Local model registry: models, aliases, guardian (symlink `config/local_models.yaml` removed 2026-09-09) | `app/local_inference/model_registry.py` via `local_models_file()` |
...
| `deploy/nginx/guardian-llmprovider-gateway-loopback-http.conf` | nginx loopback HTTP route for plain-HTTP clients; TLS trusted cert path | `deploy/tls/guardian-192.168.1.G.crt` |
...
| `deploy/tls/guardian-192.168.1.G.crt` | **TLS certificate identity** (host `guardian-192.168.1.G`) — not a service name, do not rename | `deploy/nginx/*`, `deploy/systemd/*` |
...
| Path | Function | Related processes/files |
|---|---|---|
| `config/providers/ai-node-local.settings.yaml` | Local managed provider (registry/aliases, `local: true`) | `app/proxy/providers.py`, `app/gateway/caretaker_client.py` |
...
| `config/providers/windows-gpu-local.settings.yaml` | F6 Windows/homelab passive LAN provider (`local: false`, `brand: windows`, disabled until Windows-side setup) | `app/proxy/providers.py`, F6 acceptance |
...
| `tests/unit/test_f6_remote_lan_provider.py` | F6 LAN-provider pins (local:false override, quoted-marker normalization, shipped-file markers) | `app/proxy/providers.py`, `config/providers/windows-gpu-local.settings.yaml` |
