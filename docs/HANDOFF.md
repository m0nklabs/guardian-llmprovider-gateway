...
### G2 GEFIXT op branch `f-cloud-disconnect-cap` (commit `746a90e`, 2026-08-30 avond)

- Disconnect-propagatie (niet-stream cloud) + totale duurcap per provider zijn geïmplementeerd en getest (60 + 242 groen; runtime-wiring geverifieerd). Volledige mechanismen/oordeelsbeslissingen → `docs/AGENT_JOURNAL.md` (zelfde dag, vervolg-entry).
- **Merge-volgorde voor de F7-cut-over:** PR #17 (capture-batch C1-C11, bevat ook de G1-fix) → deze G2-branch (PR volgt na de gate) → PR #16 rebasen + verse `/review` → dan pas de operator-run van de F7-checklist. Zo migreert de disconnect/cap-bug niet mee naar de nieuwe productie.
- Legacy (`llama_cpp_guardian`, pid-check 21:47 onaangetast) vertoont beide bugs tot de cut-over; backport bewust niet gedaan (legacy bevriezen).
