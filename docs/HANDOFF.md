...
- **Provider-declaratie:** een host doet mee aan speech-routing via drie regels
  ...
  control API). Volgorde = failover: `tts.providers: [14700k-local,
  ai-kvm2-local]` — **Windows is TTS-first**, ai-kvm2 (productie, 27b altijd
...
- **Caretaker-lifecycle (uniform, geen platform-gate):** `CARETAKER_TTS_COMMAND`
  ...
  vóór élke engine-start; ai-kvm2=0 → productie-27b wordt nooit weggestopt),
  `CARETAKER_TTS_MIN_FREE_MB`, `CARETAKER_TTS_VRAM_WAIT_SECONDS` (ai-kvm2=120:
...
- **Engine (qwen3tts-http, VoiceDesign 1.7B):** op Windows via caretaker
  (NSSM qwen3tts-http = disabled), op ai-kvm2 met een zelfgebouwde
...
> caretaker (teams-host), gespiegeld aan de live TTS-chain (zelfde
...
- **Speech dagelijkse validatie (operator):** de keten is synthetisch bewezen
  ...
  Audio → TTS: engine `OpenAI`, base `http://192.168.1.35:11434/v1`, model
...
- **F7 cut-over (masterplan):** F6 (Windows/14700K-provider incl. TTS) is nu
