...
- **Live bewijs:** de ensure op ai-node (27b host) kwam na ~2 min terug met `cold_start: true`; guardian-speech via Windows na idle-stop: 200 in 20,5s. Suite: caretaker 128 ✓, guardian 1419 ✓.

## 2026-09-19 — STT Route 1 live: Qwen3-ASR-1.7B via sherpa-onnx op windows-host, keten-gelijkaardig aan TTS
...
- **Guardian:** `/v1/audio/transcriptions` (OpenAI multipart) → `stt:`-config + `stt_url`-declaraties (alleen windows-gpu-local heeft een engine; ai-node-skips automatish), ISO-639-1 → Qwen-taalnamen (nl→Dutch, en→English), taal per verzoek, ensure+failover identiek aan TTS. 8 pinnen, suite 1419+ groen, gate 5/5.
...
- **Pitfalls (de moeite van het herlezen waard):**
  ...
  3. **Nooit engine-processen spawnen via ssh-Start-Process op windows-host** — ze verdwijnen stilletjes na korte tijd; de caretaker (NSSM-service) spawnen werkt urenlang stabiel. Dat is het ontwerp; gebruik het.
...
## 2026-09-23 — Huihui-Qwen3.5-9B-abliterated Q4_K_M op beide LAN-GPU-hosts + failovergroep qwen35-abliterated — DSH agent (glm-5.3-flash)

- **Wat er staat:** GGUF (5,63 GB, exact size-geverifieerd) + mmproj-F16 op ai-node (`/home/flip/models/qwen3.5-9b-abliterated/`) en windows-host (`J:\LLMSTUFF\models\`); geregistreerd in 3 config-lagen: gateway `ai-node-local.settings.yaml` (context 262144 turbo4, tensor_split 0,45/0,55, `--alias`), ai-node-caretaker `models.local.settings.yaml` (kopie + backup `.bak-20260923`), Windows-caretaker `models.local.settings.yaml` (ngl 24, 16k ctx, zelfde patroon als qwen3.5-9b daar). Nieuwe failovergroep `qwen35-abliterated` in global.settings.yaml: ai-node EERST (expliciete operator-keuze — ensure-switch disruptie geaccepteerd), windows als fallback. Alias: `qwen3.5-9b-abliterated`; route: `failover/qwen35-abliterated`.
- **E2E-bewijs:** ai-node-koude ensure 16,5 s → antwoord met backend-model `Huihui-Qwen3.5-9B-abliterated` + "ABLATED-OK"; Windows via direct adres `windows-gpu-local/windows/Huihui-Qwen3.5-9B-abliterated` → "WINDOWS-OK" (na handmatige caretaker-ensure); daarna Windows hersteld naar qwen3.5-9b via `caretaker-boot-ensure` en qwen35-groep ✓.
- **VINDING (behoefte hand-off/code-fix):** de windows-fallback-candidate is een PASSIVE provider (`managed` false) — de failover-loop (forwarding.py ~394) ensure't alleen managed providers. Tijdens de eerste rooktest serveerde windows-host stilletjes zijn geladen qwen3.5-9b als antwoord op `failover/qwen35-abliterated` — exact de "silent wrong-model" incident-klasse (2026-09-01) die de code-comments noemen. Beide hosts kunnen het model wél aan; het AUTOMATISCH omschakelen van windows-host bij failover ontbreekt alleen. Aanbeveling: in de failover-loop voor candidates met `management_url` een remote caretaker-ensure proberen (zelfde HTTP-contract als TTS/STT-ensure) vóór de forward; config-only alternatief: de operator ensure't windows-host handmatig (`POST :11441/ensure {"model":"Huihui-Qwen3.5-9B-abliterated"}`) maar dan is qwen3.5-9b daar weg tot de volgende boot-ensure — één chat-modelslot per host.
- **Les:** bij een nieuwe model-install zijn er op kvm2 TWEE copies van de modellijst (gateway provider-bestand + `/home/flip/caretaker-llamacpp/models.local.settings.yaml`); de caretaker heeft géén hot-reload → service-restart nodig (veilig zolang llama-server idle-unloaded is). Windows kent alleen de caretaker-copy.
...
## 2026-09-23 — Vervolg: remote caretaker-ensure voor passieve failover-candidates geïmplementeerd — DSH agent (glm-5.3-flash)

- **Operator wees op de kloof:** "de Windows caretaker werkt toch hetzelfde als de Linux caretaker?" — klopt, zelfde daemon (NSSM vs systemd), on-demand ensure bewezen live (`fresh_load: true`). Het gat zat puur in de gateway-failover-loop: ensure Backend krijgt het provider-object niet eens mee en spreekt alleen de LOCAL caretaker aan; passive candidates werden blind doorgestuurd.
- **Fix (`app/cloud_inference/forwarding.py`):** candidates waarvan het provider-bestand `management_url` declareert krijgen vóór de forward een remote `POST {management_url}/ensure` — zelfde daemon-contract, `management_key` uit het provider-bestand (zelfde patroon als TTS/STT-ensure), brand-prefix (`windows/`) gestript naar het kale caretaker-model-id. Faalt de ensure (onbekend model / load-fail / VRAM / unreachable) → volgende candidate; laatste attempt → eerlijke 503. Geen stille verkeerd-model-antwoorden meer.
