...
- **Goal state (operator 2026-08-26):**
  1. **Everything that serves models is a provider.** Local becomes one
     (managed) provider entry; the Windows PC (homelab) and cloud providers are
  ...
  2. **One config file per provider** (`config/providers/<name>.settings.yaml`),
     local/cloud distinction visible in the name (`ai-node-local`,
     `windows-gpu-local`, `openrouter`, `google`, …). No more defaults/overrides
     merge layer.
...
F5 caretaker-llamacpp daemon + local passive via contract (GATEWAY_MANAGER_SPLIT steps 1–2)
   │
F6 Windows/homelab provider + caretaker on Windows (LAN_GPU_BACKENDS step 2 + split step 3)
   │
...
└─ providers/
   ├─ ai-node-local.settings.yaml   (was models.local.settings.yaml + local provider block)
   ├─ windows-gpu-local.settings.yaml    (later, F6 — same shape)
...
1. `app/paths.py`: `PROVIDERS_DIR = CONFIG_DIR / "providers"`,
   ...
   keep `local_models_file()` resolving to `providers/ai-node-local.settings.yaml`
...
## F6 — Windows/homelab provider + caretaker on Windows
(`LAN_GPU_BACKENDS` step 2 + split step 3)

**Goal:** the homelab Windows GPU serves models via the LAN as a passive
...
2. caretaker-llamacpp on Windows as a Windows service (NSSM/scheduled task),
   reading its **own** `models.local.settings.yaml` (homelab GGUF paths) — no
   copy of the Linux list.
3. Firewall: open inbound ports; GGUF local on Windows (or SMB share).

### Gateway side (mostly config)
- New provider file `config/providers/windows-gpu-local.settings.yaml`
...
- `windows/…`/`windows-gpu-local/…` model addresses appear in discovery + routing
...
`windows-gpu-local/<model>` chat (non-stream + stream) → 200 with capture
...
4. Windows PC IP/port, which GGUFs, how much VRAM on the homelab.
5. Provider naming confirmed: `ai-node-local`, `windows-gpu-local`.
