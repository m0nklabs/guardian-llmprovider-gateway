...
def test_build_prefers_loopback_local_over_other_local(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config = {
        "providers": {
            ...
            "windows-gpu-local": {"local": True, "management_url": "http://192.168.1.K:11441"},
            "ai-node-local": {"local": True, "management_url": "http://127.0.0.1:11441"},
        }
    }
    client = build_caretaker_client(config)
    assert client.management_url == "http://127.0.0.1:11441"
...
def test_build_prefers_own_host_lan_ip_local(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    LAN IP (192.168.1.G), which is NOT loopback.  With a second local provider
    present, the factory must still pick THIS host's caretaker via hostname/IP
    resolution — not dict order."""
    monkeypatch.delenv("CARETAKER_KEY", raising=False)
    monkeypatch.setattr("socket.gethostname", lambda: "ai-node")
    monkeypatch.setattr(
        "socket.gethostbyname_ex",
        lambda *a: ("ai-node", ["ai-node"], ["192.168.1.G", "127.0.0.1"]),
    )
    config = {
        "providers": {
            # Intentionally ordered so the non-this-host one comes first.
            "windows-gpu-local": {"local": True, "management_url": "http://192.168.1.K:11441"},
            "ai-node-local": {"local": True, "management_url": "http://192.168.1.G:11441"},
        }
    }
    client = build_caretaker_client(config)
    assert client.management_url == "http://192.168.1.G:11441"
    import asyncio
...
def test_build_fails_closed_sending_key_to_foreign_host(
    ...
    monkeypatch.setattr(
        "socket.gethostbyname_ex",
        lambda *a: ("ai-node", ["ai-node"], ["127.0.0.1"]),
    )
    config = {
        "providers": {
            "windows-gpu-local": {"local": True, "management_url": "http://192.168.1.K:11441"},
...
def test_build_recognizes_tls_loopback_as_this_host(
    ...
    config = {
        "providers": {
            "remote-local": {"local": True, "management_url": "http://192.168.1.K:11441"},
...
def test_build_rejects_null_management_url(
    ...
    monkeypatch.delenv("CARETAKER_KEY", raising=False)
    config = {"providers": {"ai-node-local": {"local": True, "management_url": None}}}
...
def test_build_fails_closed_on_multiple_locals_no_this_host(
    ...
    monkeypatch.setattr(
        "socket.gethostbyname_ex",
        lambda *a: ("ai-node", ["ai-node"], ["127.0.0.1"]),  # no LAN IP
    )
    config = {
        "providers": {
            "windows-gpu-local": {"local": True, "management_url": "http://192.168.1.K:11441"},
