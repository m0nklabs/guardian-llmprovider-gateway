...
def test_resolve_local_and_cloud_from_provider_file_alone(monkeypatch):
    _patch(monkeypatch)
    local = sr.resolve_speech_route("guardian/14700k-local/qwen/qwen3-tts", "tts_url")
    assert local["kind"] == "local"
    assert local["tts_url"] == "http://192.168.1.245:11450"
...
async def test_explicit_local_route_ensures_and_forwards(monkeypatch):
    ...
    resp = await tts_mod.handle_audio_speech(_FakeRequest(_body(
        model="guardian/14700k-local/qwen/qwen3-tts", voice="alloy")), "dsh")
    assert resp.status_code == 200
