"""F6: the homelab Windows GPU as a passive LAN provider.

Pins the F6 provider semantics (docs/IMPLEMENTATION_PLAN.md §F6):
- an EXPLICIT ``local: false`` document marker overrides the ``-local``
  name-suffix heuristic: ``windows-gpu-local`` is a REMOTE LAN host managed by its
...
- the shipped ``windows-gpu-local.settings.yaml`` carries the markers that make it
...
def provider_dir(tmp_path: Path) -> Path:
    ...
    d.mkdir()
    (d / "windows-gpu-local.settings.yaml").write_text(textwrap.dedent("""\
        local: false
        enabled: true
    ...
    """))
    (d / "ai-node-local.settings.yaml").write_text(textwrap.dedent("""\
        local: true
        enabled: true
...
async def test_explicit_local_false_overrides_name_suffix(reg: ProviderRegistry):
    """`windows-gpu-local` + `local: false` is a passive LAN provider: NOT managed,
    configured via its api_key, cloud-routable — despite the -local suffix."""
    p = reg._providers["windows-gpu-local"]
...
async def test_local_true_still_managed(reg: ProviderRegistry):
    ...
    p = reg._providers["ai-node-local"]
    assert p is not None
    assert p.managed is True
...
async def test_cloud_models_include_windows_brand_not_managed(reg: ProviderRegistry):
    ...
    assert "ai-node-local" not in cloud
    # The managed host's models are never advertised as cloud.
    assert reg.is_cloud_model("windows/qwen3-8b-q5") is True
    assert reg.is_cloud_model("ai-node-local/llama3.2-3b") is False


def test_shipped_homelab_provider_file_markers():
    """The shipped windows-gpu-local.settings.yaml is disabled and explicitly
    ...
    path = PROVIDERS_DIR / "windows-gpu-local.settings.yaml"
...
async def test_quoted_local_marker_normalized(tmp_path: Path, monkeypatch):
    ...
    reg = ProviderRegistry()
    assert reg._providers["windows-gpu-local"].managed is False
    (d / "windows-gpu-local.settings.yaml").write_text(
        (d / "windows-gpu-local.settings.yaml").read_text().replace('"false"', '"true"')
    )
    reg2 = ProviderRegistry()
    assert reg2._providers["windows-gpu-local"].managed is True
...
async def test_unrecognized_local_string_keeps_suffix_fallback(
    ...
    d = tmp_path / "providers"
    d.mkdir()
    (d / "ai-node-local.settings.yaml").write_text(
        "local: \"flase\"\nenabled: true\nbase_url: http://127.0.0.1:11440/v1\n"
    )
    monkeypatch.setattr("app.paths.PROVIDERS_DIR", d)
    reg = ProviderRegistry()
    assert reg._providers["ai-node-local"].managed is True  # suffix fallback
    (d / "ai-node-local.settings.yaml").write_text(
        "local: \"\"\nenabled: true\nbase_url: http://127.0.0.1:11440/v1\n"
    )
    reg2 = ProviderRegistry()
    assert reg2._providers["ai-node-local"].managed is True
