...
def _cfg(**overrides):
    base = {
        "enabled": True,
        "backend_url": "http://192.168.1.W:11450",
        "timeout_seconds": 120,
        "default_instruct": "",
    }
...
async def test_success_returns_audio_wav_passthrough(monkeypatch):
    ...
    assert seen["url"] == "http://192.168.1.W:11450/tts"
...
def _cfg_ondemand(**overrides):
    base = _cfg(
        ...
        caretaker_url="http://192.168.1.W:11441",
...
async def test_ondemand_disabled_skips_ensure(monkeypatch):
    ...
    resp = await tts_mod.handle_audio_speech(_FakeRequest({"input": "x"}), "dsh")
    assert resp.status_code == 200
    assert [c["url"] for c in calls] == ["http://192.168.1.W:11450/tts"]
